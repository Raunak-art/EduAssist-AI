...
const LiquidContainer: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="min-h-screen liquid-bg flex flex-col items-center justify-center p-4 animate-in fade-in duration-300 relative overflow-hidden">
...
const App: React.FC = () => {
  ...
  // Critical: Force language attributes on document root
  useEffect(() => {
  ...
  const handleLogin = (user: User) => {
    ...
    // Skip onboarding entirely for guest mode
    if (user.isGuest) {
      setOnboardingStep('none');
    } else {
      if (!user.role) {
  ...
  if (isLoadingAuth) return <div className="h-screen w-screen flex items-center justify-center liquid-bg"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-white"></div></div>;
  if (!currentUser) return <LoginPage onLogin={handleLogin} currentLanguage={language} />;

  if (onboardingStep === 'role') return (
    <LiquidContainer>
  ...
  if (onboardingStep === 'details') return (
    <LiquidContainer>
      <div className="glass-panel p-8 rounded-3xl shadow-xl">
        ...
        <form onSubmit={handleDetailsSubmit} className="space-y-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-1 italic opacity-70">
              {tempRole === 'teacher' ? (t.ageRequirementTeacher || "Teachers must be at least 22 years old.") : (t.ageRequirementStudent || "Students must be at least 8 years old.")}
            </p>
          </div>
  ...
  return <ChatInterface key={`${currentUser.id}-${language}`} user={currentUser} onLogout={handleLogout} appLanguage={language} onLanguageChange={handleLanguageChange} />;
};
