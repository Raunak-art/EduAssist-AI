...
export const ImageModal: React.FC<ImageModalProps> = ({ imageUrl, prompt, onClose, onRecreate, language }) => {
  ...
  const handleShare = async () => {
    ...
    try {
      // 1. Get image blob from the base64 URL
      const response = await fetch(imageUrl);
      const blob = await response.blob();
      const fileName = `eduassist-art-${Date.now()}.png`;
      ...
      // 3. Fallback: Copy image to clipboard (Desktop/Other Browsers)
        ...
        try {
          ...
          console.error("Clipboard image copy failed:", clipboardError);
          // Fallback to copying the URL string if image copying is blocked
        ...
        // 4. Ultimate Fallback
        alert("Sharing not fully supported on this browser. You can right-click the image to save or copy it.");
      }
    } catch (error) {
